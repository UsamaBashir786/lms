...
try {
  $stmt = $pdo->prepare("SELECT quiz_id, teacher_id, title, description, time_limit_minutes, pass_percentage FROM quizzes WHERE quiz_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($errors)) {
    try {
      $stmt = $pdo->prepare("UPDATE quizzes SET teacher_id = ?, title = ?, description = ?, time_limit_minutes = ?, pass_percentage = ? WHERE quiz_id = ?");
      $stmt->execute([$teacher_id, $title, $description, $time_limit_minutes ?: null, $pass_percentage, $_GET['id']]);
      $success = "Quiz updated successfully.";
    } catch (PDOException $e) {
      $errors[] = "Failed to update quiz: " . $e->getMessage();
    }
  }
}
?>
          ...
          <div class="card-body">
            <form method="POST">
              <div class="mb-3">
                ...
                <select class="form-control" id="teacher_id" name="teacher_id">
                  ...
                  <?php foreach ($teachers as $teacher): ?>
                    <option value="<?php echo $teacher['id']; ?>" <?php echo $quiz['teacher_id'] == $teacher['id'] ? 'selected' : ''; ?>>
              ...
              <div class="mb-3">
                ...
                <input type="text" class="form-control" id="title" name="title" value="<?php echo htmlspecialchars($quiz['title']); ?>">
              </div>
              <div class="mb-3">
                <label for="description" class="form-label">Description</label>
                <textarea class="form-control" id="description" name="description"><?php echo htmlspecialchars($quiz['description']); ?></textarea>
              </div>
              <div class="mb-3">
                <label for="time_limit_minutes" class="form-label">Time Limit (minutes, optional)</label>
                <input type="number" class="form-control" id="time_limit_minutes" name="time_limit_minutes" value="<?php echo htmlspecialchars($quiz['time_limit_minutes']); ?>">
              </div>
              <div class="mb-3">
                <label for="pass_percentage" class="form-label">Pass Percentage</label>
                <input type="number" class="form-control" id="pass_percentage" name="pass_percentage" value="<?php echo htmlspecialchars($quiz['pass_percentage']); ?>">
              </div>
